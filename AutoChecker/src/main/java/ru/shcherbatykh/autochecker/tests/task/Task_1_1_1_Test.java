...
public class Task_1_1_1_Test extends AbstractOneClassTaskTest {
                    ...
                    (codeCheckContext, list) -> {
                        if (list.isEmpty()) {
                            ...
                            if (parameters.size() != 2
                                    || !AstUtils.isNumberType(parameters.get(0).getType(), codeCheckContext.getJavaParserFacade())
                                    || !AstUtils.isNumberType(parameters.get(1).getType(), codeCheckContext.getJavaParserFacade())) {
                                return List.of(MessageFormat.format("Неверный конструктор с параметрами. " +
                                                "Ожидалось: 2 параметра с числовым типом данных. " +
    ...
    public Task_1_1_1_Test(ClassConstructorRule classConstructorRule,
    ...
    private void checkReflectively(ClassOrInterfaceDeclaration targetClass, Class<?> clazz, RuleContext ruleContext) {
        ...
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 14, 29);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, -16, 64);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 32, -128);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, -8, -256);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 0, 0);
    }
    ...
    private void tryInvokeToStringMethod(ClassOrInterfaceDeclaration targetClass, Class<?> clazz, Field[] fields,
        ...
        List<String> expectedResults = getExpectedResults(first, second);
        ...
        if (clazzInstance == null) {
            ...
            try {
                fields[0].set(clazzInstance, first);
                fields[1].set(clazzInstance, second);
    ...
    private List<String> getExpectedResults(int first, int second) {
        if (first == second) {
            return List.of("{" + first + ";" + second + "}");
        } else {
            return List.of("{" + first + ";" + second + "}", "{" + second + ";" + first + "}");
        }
    }
