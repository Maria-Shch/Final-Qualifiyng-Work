...
@Slf4j
@Component("task_1.1.4")
@Scope("prototype")
public class Task_1_1_4_Test extends AbstractOneClassTaskTest {
    private static final int EXPECTED_AMOUNT_OF_TYPES = 2;
    private static final int EXPECTED_AMOUNT_OF_NON_MAIN_CLASSES = 1;
    private static final int EXPECTED_AMOUNT_OF_CREATED_TIMES = 3;
                    ...
                    (codeCheckContext, list) -> {
                        if (list.isEmpty()) {
                            ...
                            if (parameters.size() != 1
                                    || !AstUtils.isIntOrLongType(parameters.get(0).getType(), codeCheckContext.getJavaParserFacade())) {
                                return List.of(MessageFormat.format("Неверный конструктор с параметрами. " +
                                                "Ожидалось: 1 параметр с типом integer или long. " +
    ...
    private static final Map<String, Object> CLASS_VARIABLE_TYPES_RULE_INPUT_CONTEXT = Map.of(
            ...
            ClassVariableTypesRule.VARIABLE_2_TYPE_MAP, Map.of(ClassVariableTypesRule.ALL_VAR, "t_int_or_long")
    );
    ...
    @Autowired
    public Task_1_1_4_Test(ClassConstructorRule classConstructorRule,
    ...
    protected int getExpectedAmountOfCreatedClasses() {
        return EXPECTED_AMOUNT_OF_CREATED_TIMES;
    }
    ...
    private void checkReflectively(ClassOrInterfaceDeclaration targetClass, Class<?> clazz, RuleContext ruleContext) {
        ...
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 10);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 10000);
        tryInvokeToStringMethod(targetClass, clazz, declaredFields, clazzInstance, ruleContext, 100000);
    }

    private void tryInvokeToStringMethod(ClassOrInterfaceDeclaration targetClass, Class<?> clazz, Field[] fields,
                                         Object clazzInstance, RuleContext ruleContext, long seconds) {
        List<String> expectedResults = getExpectedResults(seconds);
        ...
        if (clazzInstance == null) {
            ...
            try {
                fields[0].set(clazzInstance, seconds);
    ...
    private List<String> getExpectedResults(long totalSeconds) {
        long oneDaySeconds = totalSeconds % 86400;
        long hours = oneDaySeconds / 3600;
        long minutes = (oneDaySeconds - hours * 3600) / 60;
        long seconds = oneDaySeconds - hours * 3600 - minutes * 60;
